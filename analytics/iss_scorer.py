...
def compute_iss(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # -------------------------------------------------------------
    vr1 = row.get("vol_ratio_1d")
    if vr1 is None:
        f4 = 0
    else:
        # Check alignment with 1d price direction
        r1d = row.get("return_1d", 0)
        is_up = r1d > 0
    ...
    # -------------------------------------------------------------
    dc = row.get("direction_consistency_20d")
    revs = row.get("intraday_reversal_count_20d")
    if dc is None:
        f6 = 0
    else:
        f6_base = int(dc * 10)
    ...
    # -------------------------------------------------------------
    dlow = row.get("distance_from_52w_low_pct")
    if dlow is None:
        f7 = 0
    elif dlow > 0.30: f7 = 5
    elif dlow > 0.10: f7 = 3
